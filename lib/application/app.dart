import 'package:flutter/material.dart';

class AssetsColor {
  ///主题色
  static final Color COLOR_PRIMARY = Colors.lightBlue;
  static final Color COLOR_BACKGROUND = Colors.grey.shade100;
  static final Color COLOR_TITLE_TEXT = Colors.black.withOpacity(0.7);
  static final Color COLOR_SUBTITLE_TEXT = Colors.grey.withOpacity(0.9);
  static final Color COLOR_CONTENT_TEXT = Colors.black.withOpacity(0.7);
}

class AssetsSize {
  static final double SIZE_DEFAULT_PADDING = 14;
  static final double SIZE_DEFAULT_DISTANCE = 16;
  static final double SIZE_ICON = 24;
  static final double SIZE_BIG_TITLE_TEXT = 18;
  static final double SIZE_TITLE_TEXT = 16;
  static final double SIZE_SUBTITLE_TEXT = 14;
  static final double SIZE_CONTENT_TEXT = 14;
  static final double SIZE_MINI_TEXT = 12;
}
